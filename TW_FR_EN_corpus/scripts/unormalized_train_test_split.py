...
if __name__ == "__main__":
    ...
    if (args.val_set == True):
        ...
        preprocessor.writeTotxt(
            f'{args.train_ouput_path}/un_train_{args.id_tw}.txt', train_twi)
        preprocessor.writeTotxt(
            f'{args.train_ouput_path}/un_train_{args.id_en}.txt', train_en)
        preprocessor.writeTotxt(
            f'{args.test_ouput_path}/un_test_{args.id_tw}.txt', test_twi)
        preprocessor.writeTotxt(
            f'{args.test_ouput_path}/un_test_{args.id_en}.txt', test_en)
        preprocessor.writeTotxt(
            f'{args.train_ouput_path}/un_train_{args.id_fr}.txt', train_fr)
        preprocessor.writeTotxt(
            f'{args.test_ouput_path}/un_test_{args.id_fr}.txt', test_fr)
        preprocessor.writeTotxt(
            f'{args.val_ouput_path}/un_val_{args.id_fr}.txt', val_fr)
        preprocessor.writeTotxt(
            f'{args.val_ouput_path}/un_val_{args.id_tw}.txt', val_twi)
        preprocessor.writeTotxt(
            f'{args.val_ouput_path}/un_val_{args.id_en}.txt', val_en)
    else:
        train_twi, test_twi, train_en, test_en, train_fr, test_fr = train_test_split(
    ...
    print("SUCCESS SPLITED CORPUS WITHOUT NORMALIZATION")